...
Selection rides on the existing per-project `.cc-suite.md` config (already generated by `/cc-suite:init`). It carries an `## Enabled Tools` section written as a Markdown task list (checked = enabled). The task-list form is used instead of a `# - tool` comment because a commented list item is ambiguous with a Markdown heading and would break section parsing:
