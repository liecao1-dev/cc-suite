---
name: claude
description: "显式把任意问题或工作派遣给 Claude Code。当用户单独提交 $claude 或在 $claude 后接任务时使用；每次都先让用户选择模型、推理强度和权限配置，再在缺少任务时询问任务。"
---

# Claude

`$claude` 和 `$claude <任务>` 是 Codex → Claude 的日常派遣入口。用户可以先
打开配置选择，再用大白话补充任务；不要要求 `review`、`plan`、`implement`、
`debug` 等工作流分类。
...
- 用户提交 `$claude` 后，第一项用户可见交互必须是配置选择；即使尚无任务，
  也不得先询问任务。
...
用户对配置选择问题的回答和随后补充的任务都属于本次尚未发出的派遣，不要求
重复 `$claude`。
...
这一步必须是提交 `$claude` 后的第一项用户可见交互。不要先询问任务，也不要因
初始消息已经包含任务而跳过配置选择。
...
- `$claude` 后有内容时，按原意保存为 `{task}`，不要重新询问。
- 用户只提交 `$claude` 时，询问“这次要 Claude 做什么？”，把回答按原意保存为
  `{task}`。用户无需再次输入 `$claude`。
- 任务为空或用户取消时停止，不记录本次配置，也不调用 Claude MCP。

在任务和配置都明确前不要调用任何 Claude MCP 工具。
...
最后提醒：下一次派遣请重新输入 `$claude` 或 `$claude <任务>`。
...
只提交 `$claude` 时先选配置、再补任务；其余三种写法先选配置，然后直接派遣
已经附带的自然语言任务。
